...
public interface FlightRepository extends JpaRepository<Flight, Integer>,PagingAndSortingRepository<Flight, Integer>{
	...
	@Query(value="select * from flight where departure== :departure and arrival== :arrival and departureSource== :departureSource"
			+ "and arrivalDestination", nativeQuery=true)
	...
	@Query(value="select * from flight where departure== :departure and arrival== :arrival and departureSource== :departureSource"
			+ "and arrivalDestination and flightCompany==:flightCompany" , nativeQuery=true)
	List<Flight> serachFlightByFlightName(DateTimeFormatter departure,DateTimeFormatter arrival,String departureSource,String arrivalDestination,String flightComapny);
