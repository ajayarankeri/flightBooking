...
public interface FlightRepository extends JpaRepository<Flight, Long>,PagingAndSortingRepository<Flight, Long>{
	...
	@Query(value="select * from flights where departure_time= (:departure) and arrival_time= (:arrival) and source= (:source)"
			+ "and destination=(:destination) and flight_status=1", nativeQuery=true)
	List<Flight> searchFlight( LocalDateTime departure,LocalDateTime arrival,String source ,String destination);
	
	@Query(value="select * from flights where departure_time= (:departure) and arrival_time= (:arrival) and source= (:source)"
			+ "and destination= (:destination) and flight_company= (:flightCompany) and flight_status=1" , nativeQuery=true)
	...
	List<Flight> findByFlightStatus(int i);
